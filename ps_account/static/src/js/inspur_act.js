...
ListRenderer.include({
    ...
    events: _.extend({}, ListRenderer.prototype.events, {
        ...
        'click tr .o_list_record_remove .fa-minus-square': '_onRemoveIconClick',
        'click tr .o_list_record_remove .fa-trash-o': '_onRemoveIconClick',
        'click .o_field_x2many_list_row_add': '_onAddRecord',
        'click .o_field_x2many_list_row_add a': '_onAddRecord',
        'click .o_field_x2many_list_row_add .fa-plus-square': '_onAddRecord',
        'click tr .o_list_record_remove .fa-plus-square': '_onAddRecord',
    }),
    /**
     * @override
     * @param {Object} params
    ...
    _renderRow: function (record, index) {
        ...
        if (this.addTrashIcon) {
            if(this.el.className=="inspur_account table-responsive"){
                var $icon1 = $('<span>', {class: 'fa fa-2x fa-plus-square', name: 'add'});
                var $icon2 = $('<span>', {class: 'fa fa-2x fa-minus-square', name: 'delete'});
                var $td = $('<td>', {class: 'o_list_record_remove'}).append($icon1);
                $td=$td.append($icon2);
                $td.css('opacity','0');
                $td.attr("id","icon_border");
                $row.append($td);
    ...
    _renderRows: function () {
        ...
        if (this.addCreateLine) {
            if(this.el.className=="inspur_account table-responsive"){
                ...
                var $tr = $('<tr>',{class:"aaa"});
                for(var i=0;i<this._getNumberOfCols();i++){
                    var $td=$('<td>').attr('colspan',"1").addClass('o_field_x2many_list_row_add')
                    $tr.append($td);
                    if(i==0){
                        $td=$td.append($a);
                    }
                    if(i==(this._getNumberOfCols()-3)||i==(this._getNumberOfCols()-2)){
                        $td.attr("class","o_field_x2many_list_row_add inspur_number")
                    }
                    $rows.push($tr);
                }
                $tr.children().last().append($('<span>', {class: 'fa fa-2x fa-plus-square', name: 'add'}));
                // $tr.children().last().append($('<span>', {class: 'fa fa-2x fa-minus-square', name: 'delete'}));
                $tr.children().last().attr("id","icon-ctrl").css("opacity",'0');
                $tr.mouseover(function(){
                    $tr.children().last().css("opacity",'1');
                })
                $tr.mouseout(function(){
                    $tr.children().last().css("opacity",'0');
                })
            }else{
                var $a = $('<a href="#">').text(_t("Add an item"));
                var $td = $('<td>')
