...
var ListRenderer = BasicRenderer.extend({
    ...
    _renderAggregateCells: function (aggregateValues, isHeader) {
        ...
        return _.map(this.columns, function (column, index) {
            ...
            if (column.attrs.name in aggregateValues) {
                ...
                var judge=formattedValue[0];
                if(judge=="-"){
                    formattedValue=formattedValue.substr(1,formattedValue.length-1);
    ...
    _renderBody: function () {
        ...
        if($rows.length>0){
            for(var i=0; i<$rows.length; i++){
                $rows[i]
                if(this.el.className=="ps_stock_picking_move account_invoices table-responsive" || this.el.className=="ps_purchase_stock_verification stock_account_moves table-responsive"){
                    $rows[i].find(".o_data_cell").last().attr("class", "ps_display");
                }
            }
        }
        while ($rows.length < 1) {
    ...
    _renderBodyCell: function (record, node, colIndex, options) {
        ...
        if($div.parent().hasClass("o_data_cell o_list_number rel") || $div.parent().hasClass("o_data_cell o_list_number o_readonly_modifier rel")){
            // var rel=formattedValue[1];
            var reld=res.substr(res.length-1,1);
            var reg = /^[0-9]+.?[0-9]*$/;
            if(reg.test(reld)){
                var rel=formattedValue[1];
                if(reg.test(rel)){
                    ...
                    res=res.substr(2,res.length-1);
                    if(rel=='-'){
                        $div.css("color","red");
                    }
                }
            }else{
                var rel=formattedValue[0];
                if(reg.test(rel)){
    ...
    _renderFooter: function () {
        ...
        var $tfoot= $('<tfoot>').append($('<tr>').append($cells));
        if($cells.length>3){
            if($cells[3].attr("title")=="Total Debit"||$cells[3].attr("title")=="借方合计"){
                var realval=$cells[3].html();
                var sym=$cells[3].attr("id");
                var psFunc=realval.replace(",","");
                psFunc=psFunc.replace(",","");
                 //---字符处理完毕，开始转换，转换采用前后两部分分别转换---//
                var part = String(psFunc).split(".");
                var newchar = "";
                //小数点前进行转化
                for ( var i = part[0].length - 1; i >= 0; i--) {
                    if (part[0].length > 9) { alert(_t("The number of digits is too large to calculate")); return ""; } //若数量超过亿单位，提示
                    var tmpnewchar = ""
                ...
                var $div=$("<div>",{class:"upcase"});
                if(newchar!="元整") {
                    if (sym == "minus") {
                        $div.html(_t("Total (uppercase): negative") + newchar);
                    } else {
                        $div.html(_t("Total (uppercase):") + newchar);
                    }
                }
                $tfoot=$tfoot.append($div);
            }
        }
        return $tfoot;
    },
    ...
    _renderHeader: function (isGrouped) {
        ...
        $tr.append($('<th>').html("&nbsp;").css("width","36px"));
        if(this.el.className=="ps_stock_picking_move account_invoices table-responsive" || this.el.className=="ps_purchase_stock_verification stock_account_moves table-responsive"){
            $tr.find(".o_column_sortable").last().css("display","none");
        }
    ...
    _renderHeaderCell: function (node) {
        ...
        if($th.text()=="借方金额"||$th.text()=="贷方金额"){
            $th.css("width","222px");
            var $span=$('<span>',{class:"unit_china"});
            $span.html("亿千百十万千百十元角分")
            $th=$th.append($span);
        }
    ...
    _renderView: function () {
        ...
        //只在凭证页面显示序号
        if($table.find("tr")[0].firstChild.getAttribute("class")=="o_list_inspur_number"||$table.find("tr")[0].firstChild.getAttribute("class")=="o_list_row_number_header"){
            if($table.parent().attr("class")!="inspur_account table-responsive"){
                var arr = $table.find("tr");
                var arr_d = $table.find(".o_group_header");
        ...
        //借贷切换
        $(".rel_debit input").bind("input propertychange",function () {
                        console.log("Already entered");
                    });
        $(".rel_credit input").bind("input propertychange",function () {
                        console.log("Already entered");
                    });
        if(this.editable!==false){
            document.onkeydown=function(e){
                if (e.keyCode == 32){
                    if(e.target.parentNode.classList.contains("rel")){
                        var num1=$(".rel_debit input").val();
                        var num2=$(".rel_credit input").val();
                        $(".rel_debit input").val(num2);
                        $(".rel_credit input").val(num1);
                        $(".rel_debit input").trigger("input");
                        $(".rel_credit input").trigger("input");
                    }
                }
            }
        ...
        //库存生成凭证页面增加复选框
        if($table.parent().hasClass("ps_stock_picking_move")){
            var $th = $("<th>").append($("<input>",{type:"checkbox",name:"check"}))
            $table.find("tr").prepend($th)
            if($table.find("tbody").find("td").hasClass("o_data_cell")!=true){
                $table.find("tbody").find("tr").children(":first").css("display","none")
            }
            $table.find("tfoot").find("tr").children(":first").css("display","none")
            var num=true
            $table.find("thead").children(":first").click(function(){
                if(num){
        ...
        //采购发票与入库单核销
        if($table.parent().hasClass("ps_purchase_stock_verification")){
            var $th = $("<th>").append($("<input>",{type:"checkbox",name:"stockcheck"}))
            $table.find("tr").prepend($th)
            if($table.find("tbody").find("td").hasClass("o_data_cell")!=true){
                $table.find("tbody").find("tr").children(":first").css("display","none")
            }
            ...
            $table.find("thead").children(":first").click(function(){
                if(num){
                    $('input[name="stockcheck"]').prop('checked',true);
                }else{
                    $('input[name="stockcheck"]').prop('checked',false);
                }
                num=!num
            })
            // $('input[name="check"]').prop('checked','true');
        ...
        if($table.parent().hasClass("o_form_view_inspur inspur_check")){
            var aa=$table[0];
            if(aa.rows.length>3){
                if($table.find("thead").find("tr")[0].childNodes[0].getAttribute("class")=="o_list_row_number_header") {
                    autoRowSpan(aa, 0, 2);
                }else{
                    autoRowSpan(aa, 0, 1);
                }
            ...
            for(var i=0;i<$table.find("tr").length;i++){
                if($table.find("thead").find("tr")[0].childNodes[0].getAttribute("class")=="o_list_row_number_header"){
                    $table.find("tr")[i].firstChild.style.display="none";
                }
            }
            for(var n=0;n<$table.find("tbody").find("tr").length;n++){
                if($table.find("tbody").find("tr")[n].childNodes[0].getAttribute("class")=="fff"){
                    if($table.find("tbody").find("tr")[n].childElementCount==12){
                       $table.find("tbody").find("tr")[n].childNodes[1].style.display="none";
                       $table.find("tbody").find("tr")[n].childNodes[1].childNodes[0].childNodes[0].setAttribute("disabled","disabled");
                    }
                    if($table.find("tbody").find("tr")[n].childElementCount==13){
                       var num=$table.find("tbody").find("tr")[n].childNodes[2].getAttribute("rowspan");
                       $table.find("tbody").find("tr")[n].childNodes[1].setAttribute("rowspan",num);
                    }
                }else{
                    if($table.find("tbody").find("tr")[n].childElementCount==11){
                       $table.find("tbody").find("tr")[n].childNodes[0].style.display="none";
                       $table.find("tbody").find("tr")[n].childNodes[0].childNodes[0].childNodes[0].setAttribute("disabled","disabled");
                    }
                    if($table.find("tbody").find("tr")[n].childElementCount==12){
                       var num=$table.find("tbody").find("tr")[n].childNodes[1].getAttribute("rowspan");
                       $table.find("tbody").find("tr")[n].childNodes[0].setAttribute("rowspan",num);
                    }
                }
            }
        }
        function autoRowSpan(aa,row,col){
            var lastValue="";
            var value="";
            var lastValuedate="";
            var valuedate="";
            var pos=1;
            for(var i=row;i<aa.rows.length;i++){
                value = aa.rows[i].cells[col].innerText;
                valuedate = aa.rows[i].cells[col+2].innerText;
                if(lastValue == value && lastValuedate == valuedate){
                    aa.rows[i].deleteCell(col);
                    aa.rows[i-pos].cells[col].rowSpan = aa.rows[i-pos].cells[col].rowSpan+1;
                    pos++;
                }else{
                    lastValue = value;
    ...
    _onToggleSelection: function (event) {
        ...
        if(checked===true){
            for(var i=0;i<this.$('tbody .o_list_record_selector input').length;i++) {
                var num = this.$('tbody .o_list_record_selector input')[i].disabled;
                if (num === false) {
                    this.$('tbody .o_list_record_selector input').eq(i).prop('checked', checked);
                }
            }
        }else{
