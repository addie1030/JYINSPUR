# -*- coding: utf-8 -*-

import json

from odoo import api, models, _
from odoo.tools import float_round


class ReportMaterialsForwardQuery(models.AbstractModel):
    _name = 'report.ps_production_data.report_mfq_structure'
    _description = 'Report of Materials Forward Query'

    @api.model
    def _get_report_values(self, docids, data=None):
        docs = []
        for bom_id in docids:
            bom = self.env['mrp.bom'].browse(bom_id)
            variant = data and data.get('variant')
            candidates = variant and self.env['product.product'].browse(variant) or bom.product_tmpl_id.product_variant_ids
            for product_variant_id in candidates:
                if data and data.get('childs'):
                    doc = self._get_pdf_line(bom_id, product_id=product_variant_id, qty=float(data.get('quantity')), child_bom_ids=json.loads(data.get('childs')))
                else:
                    doc = self._get_pdf_line(bom_id, product_id=product_variant_id, unfolded=True)
                doc['report_type'] = 'pdf'
                doc['report_structure'] = data and data.get('report_type') or 'all'
                docs.append(doc)
            if not candidates:
                if data and data.get('childs'):
                    doc = self._get_pdf_line(bom_id, qty=float(data.get('quantity')), child_bom_ids=json.loads(data.get('childs')))
                else:
                    doc = self._get_pdf_line(bom_id, unfolded=True)
                doc['report_type'] = 'pdf'
                doc['report_structure'] = data and data.get('report_type') or 'all'
                docs.append(doc)
        return {
            'doc_ids': docids,
            'doc_model': 'mrp.bom',
            'docs': docs,
        }

    @api.model
    def get_bom_versions_by_productid(self, searchProduct=False):
        bom_versions = {}
        module_line = self.env['ir.module.module'].search([('name', '=', 'mrp_plm')])
        if module_line:
            if module_line.state == 'installed':
                is_bom_version_applied = True
            else:
                is_bom_version_applied = False
        else:
            is_bom_version_applied = False

        if is_bom_version_applied:
            bom_version_ids = self.env['mrp.bom'].search([('product_tmpl_id', '=', searchProduct)], order='id')
            for bom_record in bom_version_ids:
                bom_versions[bom_record.version] = bom_record.version
        return [bom_versions,is_bom_version_applied]

    @api.model
    def get_bom_by_version_productid(self, searchProduct=False, searchBomVersion=False):
        module_line = self.env['ir.module.module'].search([('name', '=', 'mrp_plm')])
        if module_line:
            if module_line.state == 'installed':
                is_bom_version_applied = True
            else:
                is_bom_version_applied = False
        else:
            is_bom_version_applied = False

        if is_bom_version_applied:
            bom_ids = self.env['mrp.bom'].search([('product_tmpl_id', '=', searchProduct), ('version', '=', searchBomVersion)], order='id')
        else:
            bom_ids = self.env['mrp.bom'].search(
                [('product_tmpl_id', '=', searchProduct)], order='id')

        return bom_ids[0].id

    @api.model
    def get_html(self, bom_id=False, searchQty=1, searchProduct=False, searchBomVersion=False, searchLayer='mlayer'):
        if not bom_id:
            if not searchBomVersion:
                bom_ids = self.env['mrp.bom'].search([], order='id')
            else:
                bom_ids = self.env['mrp.bom'].search([('version', '=', searchBomVersion)], order='id')
        else:
            if not searchBomVersion:
                bom_ids = self.env['mrp.bom'].search(
                    [('product_tmpl_id', '=', bom_id)], order='id')
            else:
                bom_ids = self.env['mrp.bom'].search([('product_tmpl_id', '=', bom_id), ('version', '=', searchBomVersion)], order='id')
        if bom_ids:
            bom_id = bom_ids[0].id
            searchProduct = self.env['product.template'].search([('id','=',bom_ids[0].product_tmpl_id.id)]).id

        res = self._get_report_data(bom_id=bom_id, searchQty=searchQty, searchProduct=searchProduct, searchBomVersion=searchBomVersion, searchLayer=searchLayer)

        res['lines']['report_type'] = 'html'
        res['lines']['report_structure'] = 'all'
        if not res['lines'].get('attachments'):
            res['lines']['has_attachments'] = False
        else:
            res['lines']['has_attachments'] = res['lines']['attachments'] or any(component['attachments'] for component in res['lines']['components'])
        res['lines'] = self.env.ref('ps_production_data.report_mfq_bom').render({'data': res['lines']})
        return res

    @api.model
    def get_bom(self, bom_id=False, product_id=False, line_qty=False, line_id=False, level=False):
        lines = self._get_bom(bom_id=bom_id, product_id=product_id, line_qty=line_qty, line_id=line_id, level=level)
        return self.env.ref('ps_production_data.report_mfq_bom_line').render({'data': lines})

    @api.model
    def get_operations(self, bom_id=False, qty=0, level=0):
        bom = self.env['mrp.bom'].browse(bom_id)
        if not bom:
            return
        lines = self._get_operation_line(bom.routing_id, float_round(qty / bom.product_qty, precision_rounding=1, rounding_method='UP'), level)
        values = {
            'bom_id': bom_id,
            'currency': self.env.user.company_id.currency_id,
            'operations': lines,
        }
        return self.env.ref('mrp.report_mrp_operation_line').render({'data': values})

    def _get_bom_reference(self, bom):
        return bom.display_name

    @api.model
    def _get_report_data(self, bom_id, searchQty=0, searchProduct=False, searchBomVersion=False, searchLayer='mlayer'):
        lines = {}
        if not searchBomVersion:
            bom = self.env['mrp.bom'].browse(bom_id)
        else:
            bom = self.env['mrp.bom'].search([('id', '=', bom_id), ('version', '=', searchBomVersion)])
        bom_quantity = searchQty or bom.product_qty
        bom_product_variants = {}
        bom_versions = {}
        bom_uom_name = ''

        if bom:
            bom_uom_name = bom.product_uom_id.name

            # Get variants used for search
            if not bom.product_id:
                for variant in bom.product_tmpl_id.product_variant_ids:
                    bom_product_variants[variant.id] = variant.display_name

        if not searchBomVersion:
            bom_product_ids = self.env['mrp.bom'].search([], order='id')
        else:
            bom_product_ids = self.env['mrp.bom'].search([('version', '=', searchBomVersion)], order='id')

        bom_product_variants = {}
        for bom_record in bom_product_ids:
            if bom_record.product_tmpl_id not in bom_product_variants.keys():
                bom_product_variants[bom_record.product_tmpl_id] = bom_record.product_tmpl_id.display_name

        bom_versions_by_productid = self.get_bom_versions_by_productid(bom.product_tmpl_id.id)
        bom_versions = bom_versions_by_productid[0]
        is_bom_version_applied = bom_versions_by_productid[1]

        bom_product_variants = {}
        bom_ids = self.env['mrp.bom'].search([], order='id')
        for bom_record in bom_ids:
            if bom_record.display_name not in bom_product_variants.values():
                bom_product_variants[bom_record.product_tmpl_id.id] = bom_record.display_name

        lines = self._get_bom(bom_id, product_id=bom.product_tmpl_id.id, line_qty=bom_quantity, level=1, searchBomVersion=searchBomVersion, searchLayer=searchLayer)
        if not lines:
            lines = {}

        return {
            'lines': lines,
            'variants': bom_product_variants,
            'bom_uom_name': bom_uom_name,
            'bom_qty': bom_quantity,
            'bom_prod_name': bom.product_tmpl_id.display_name,
            'is_variant_applied': self.env.user.user_has_groups('product.group_product_variant') and len(bom_product_variants) > 1,
            'is_uom_applied': self.env.user.user_has_groups('uom.group_uom'),
            'is_bom_version_applied': is_bom_version_applied,
            'bom_versions': bom_versions
        }

    def _get_bom(self, bom_id=False, product_id=False, line_qty=False, line_id=False, level=False, searchBomVersion=False, searchLayer='mlayer'):
        if not searchBomVersion:
            bom = self.env['mrp.bom'].browse(bom_id)
        else:
            bom = self.env['mrp.bom'].search([('id', '=', bom_id), ('version', '=', searchBomVersion)])
        if not bom:
            return
        bom_quantity = line_qty
        if line_id:
            current_line = self.env['mrp.bom.line'].browse(int(line_id))
            bom_quantity = current_line.product_uom_id._compute_quantity(line_qty, bom.product_uom_id)
        # Display bom components for current selected product variant

        if product_id:
            if isinstance(product_id, str):
                product = self.env['product.product'].search([('product_tmpl_id', '=', int(product_id))])
            elif isinstance(product_id, int):
                product = self.env['product.product'].search([('product_tmpl_id', '=', product_id)])
            else:
                product = self.env['product.product'].search([('product_tmpl_id', '=', int(product_id.id))])
        else:
            product = bom.product_id or bom.product_tmpl_id.product_variant_id

        if product:
            attachments = self.env['mrp.document'].search(['|', '&', ('res_model', '=', 'product.product'),
            ('res_id', '=', product.id), '&', ('res_model', '=', 'product.template'), ('res_id', '=', product.product_tmpl_id.id)])
        else:
            product = bom.product_tmpl_id
            attachments = self.env['mrp.document'].search([('res_model', '=', 'product.template'), ('res_id', '=', product.id)])
        operations = self._get_operation_line(bom.routing_id, float_round(bom_quantity / bom.product_qty, precision_rounding=1, rounding_method='UP'), 0)
        # mrp_bom_line.ps_is_quantitative/mrp_bom.routing_id/mrp_bom_line.operation_id/mrp_bom_line.ps_attrition_rate/mrp_bom.product_qty/mrp_bom_line.product_qty
        if not searchLayer:
            searchLayer = 'mlayer'
        if not searchBomVersion:
            searchBomVersion = '1'

        lines = {
            'bom': bom,
            'bom_qty': bom_quantity,
            'bom_prod_name': product.display_name or '',
            'currency': self.env.user.company_id.currency_id,
            'product': product,
            'code': bom and self._get_bom_reference(bom) or '',
            'price': product.uom_id._compute_price(product.standard_price, bom.product_uom_id) * bom_quantity,
            'total': sum([op['total'] for op in operations]),
            'level': level or 0,
            'operations': operations,
            'operations_cost': sum([op['total'] for op in operations]),
            'attachments': attachments,
            'operations_time': sum([op['duration_expected'] for op in operations]),
            'ps_is_quantitative': False,
            'routing': bom.routing_id,
            'routing_name': bom.routing_id.display_name,
            'operation_id': '',
            'operation_name': '',
            'ps_attrition_rate': 0.0,
            'parent_product_qty': bom_quantity,
            'sub_product_qty': bom_quantity,
            'search_bom_version': searchBomVersion,
            'search_layer': searchLayer
        }
        components, total = self._get_bom_lines(bom, bom_quantity, product, line_id, level, searchBomVersion, searchLayer)
        lines['components'] = components
        lines['total'] += total
        return lines

    def _get_bom_lines(self, bom, bom_quantity, product, line_id, level, searchBomVersion=False, searchLayer='mlayer'):
        components = []
        total = 0
        for line in bom.bom_line_ids:
            line_quantity = (bom_quantity / (bom.product_qty or 1.0)) * line.product_qty
            if line._skip_bom_line(product):
                continue
            price = line.product_id.uom_id._compute_price(line.product_id.standard_price, line.product_uom_id) * line_quantity
            if line.child_bom_id:
                factor = line.product_uom_id._compute_quantity(line_quantity, line.child_bom_id.product_uom_id) / line.child_bom_id.product_qty
                sub_total = self._get_price(line.child_bom_id, factor, line.product_id)
            else:
                sub_total = price
            sub_total = self.env.user.company_id.currency_id.round(sub_total)
            if not searchLayer:
                searchLayer = 'mlayer'
            if not searchBomVersion:
                searchBomVersion = '1'
            components.append({
                'prod_id': line.product_id.id,
                'prod_name': line.product_id.display_name,
                'code': line.child_bom_id and self._get_bom_reference(line.child_bom_id) or '',
                'prod_qty': line_quantity,
                'prod_uom': line.product_uom_id.name,
                'prod_cost': self.env.user.company_id.currency_id.round(price),
                'parent_id': bom.id,
                'line_id': line.id,
                'level': level or 0,
                'total': sub_total,
                'child_bom': line.child_bom_id.id,
                'phantom_bom': line.child_bom_id and line.child_bom_id.type == 'phantom' or False,
                'attachments': self.env['mrp.document'].search(['|', '&',
                    ('res_model', '=', 'product.product'), ('res_id', '=', line.product_id.id), '&', ('res_model', '=', 'product.template'), ('res_id', '=', line.product_id.product_tmpl_id.id)]),
                'ps_is_quantitative': line.ps_is_quantitative,
                'routing': line.routing_id,
                'routing_name': line.routing_id.display_name,
                'operation_id': line.operation_id,
                'operation_name': line.operation_id.display_name,
                'ps_attrition_rate': line.ps_attrition_rate,
                'parent_product_qty': bom_quantity,
                'sub_product_qty': line_quantity,
                'search_bom_version': searchBomVersion,
                'search_layer': searchLayer
            })
            total += sub_total
        return components, total

    def _get_operation_line(self, routing, qty, level):
        operations = []
        total = 0.0
        for operation in routing.operation_ids:
            operation_cycle = float_round(qty / operation.workcenter_id.capacity, precision_rounding=1, rounding_method='UP')
            duration_expected = operation_cycle * operation.time_cycle + operation.workcenter_id.time_stop + operation.workcenter_id.time_start
            total = ((duration_expected / 60.0) * operation.workcenter_id.costs_hour)
            operations.append({
                'level': level or 0,
                'operation': operation,
                'name': operation.name + ' - ' + operation.workcenter_id.name,
                'duration_expected': duration_expected,
                'total': self.env.user.company_id.currency_id.round(total),
            })
        return operations

    def _get_price(self, bom, factor, product):
        price = 0
        if bom.routing_id:
            # routing are defined on a BoM and don't have a concept of quantity.
            # It means that the operation time are defined for the quantity on
            # the BoM (the user produces a batch of products). E.g the user
            # product a batch of 10 units with a 5 minutes operation, the time
            # will be the 5 for a quantity between 1-10, then doubled for
            # 11-20,...
            operation_cycle = float_round(factor, precision_rounding=1, rounding_method='UP')
            operations = self._get_operation_line(bom.routing_id, operation_cycle, 0)
            price += sum([op['total'] for op in operations])

        for line in bom.bom_line_ids:
            if line._skip_bom_line(product):
                continue
            if line.child_bom_id:
                qty = line.product_uom_id._compute_quantity(line.product_qty * factor, line.child_bom_id.product_uom_id)
                sub_price = self._get_price(line.child_bom_id, qty, line.product_id)
                price += sub_price
            else:
                prod_qty = line.product_qty * factor
                not_rounded_price = line.product_id.uom_id._compute_price(line.product_id.standard_price, line.product_uom_id) * prod_qty
                price += self.env.user.company_id.currency_id.round(not_rounded_price)
        return price

    def _get_pdf_line(self, bom_id, product_id=False, qty=1, child_bom_ids=[], unfolded=False):

        data = self._get_bom(bom_id=bom_id, product_id=product_id, line_qty=qty)

        def get_sub_lines(bom, product_id, line_qty, line_id, level):
            data = self._get_bom(bom_id=bom.id, product_id=product_id, line_qty=line_qty, line_id=line_id, level=level)
            bom_lines = data['components']
            lines = []
            for bom_line in bom_lines:
                lines.append({
                    'name': bom_line['prod_name'],
                    'type': 'bom',
                    'quantity': bom_line['prod_qty'],
                    'uom': bom_line['prod_uom'],
                    'prod_cost': bom_line['prod_cost'],
                    'bom_cost': bom_line['total'],
                    'level': bom_line['level'],
                    'code': bom_line['code'],
                    'ps_is_quantitative': bom_line['ps_is_quantitative'],
                    'routing': bom_line['routing'],
                    'routing_name': bom_line['routing_name'],
                    'operation_id': bom_line['operation_id'],
                    'operation_name': bom_line['operation_name'],
                    'ps_attrition_rate': bom_line['ps_attrition_rate'],
                    'parent_product_qty': bom_line['parent_product_qty'],
                    'sub_product_qty': bom_line['sub_product_qty'],
                    'search_bom_version': bom_line['search_bom_version'],
                    'search_layer': bom_line['search_layer']
                })
                if bom_line['child_bom'] and (unfolded or bom_line['child_bom'] in child_bom_ids):
                    line = self.env['mrp.bom.line'].browse(bom_line['line_id'])
                    lines += (get_sub_lines(line.child_bom_id, line.product_id, bom_line['prod_qty'], line, level + 1))

            return lines

        bom = self.env['mrp.bom'].browse(bom_id)
        product = product_id or bom.product_id or bom.product_tmpl_id.product_variant_id
        pdf_lines = get_sub_lines(bom, product, qty, False, 1)
        data['components'] = []
        data['lines'] = pdf_lines
        return data
