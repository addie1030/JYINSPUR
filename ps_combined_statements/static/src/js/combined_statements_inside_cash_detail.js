odoo.define('combined_statements_inside_cash_detail_tree_btn', function (require) {
	"use strict";
	...
	function execute_open_action() {
		let build_data = function () {
			...
			new Model('ps.combined.statements.inside.cash.detail').call('builder_inside_cash_detail', [organization, period]).then(function (result) {
				if (result) {
		...
		new Model("ps.combined.statements.organization").query(['name', 'code']).filter([['is_entity_company', '=', true]]).all().then(function (result) {
			if (result) {
				new Dialog(this, {
					...
					$content: $(QWeb.render('InsideCashDetailInfo', {widget: this, data: result}))
				}).open();
			}
		});
	}
	...
	ListView.include({
		render_buttons: function () {
			...
			if (add_button) {
				this.$buttons.on('click', '.o_list_inside_cash_detail', execute_open_action.bind(this));
			}
		}
	});
